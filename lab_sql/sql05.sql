/*
 * 서브쿼리(subquery): SQL 문장의 일부로 다른 SQL 문장이 포함되는 것. 서브쿼리에서는 세미콜론(;) 쓰면 안됨!
 */
 
 -- 급여가 평균 이상인 직원들?
 select avg(sal) from emp; --> avg(sal) 2073.214285
 select * from emp where sal >= 2073.21;
 -- select * from emp where sal >= avg(sal); (X)
 
 select * from emp where sal >=
    (select avg(sal) from emp); -- (select avg(sal) from emp) 서브쿼리

-- ALLEN보다 더 적은 급여를 받는 직원들의 사번, 이름, 급여를 검색
select empno, ename, sal from emp where sal <
    (select sal from emp where ename = 'ALLEN'); -- 1600

-- ALLEN과 같은 업무를 담당하는 직원들의 사번, 이름, 부서번호, 업무, 급여를 검색.
...
select empno, ename, sal, job from emp where sal >
...
-- 연봉 내림차순 정렬.
select ename, sal, nvl(comm, 0) as comm, sal * 12 + nvl(comm, 0) as ANNUAL_SAL from emp where sal * 12 + nvl(comm, 0) >
    (select sal * 12 + nvl(comm, 0) from emp where ename = 'WARD') -- 15,500
        order by ANNUAL_SAL desc;

-- Ex. SCOTT과 같은 급여를 받는 직원들의 이름과 급여를 검색.
-- Ex. 위 결과에서 SCOTT은 제외하고 검색.
select ename, sal from emp where ename != 'SCOTT' and sal = 
    (select sal from emp where ename = 'SCOTT'); -- 3,000

-- Ex. ALLEN보다 늦게 입사한 직원들의 이름, 입사날짜를 최근입사일부터 출력.
select ename, hiredate from emp where hiredate > 
    (select hiredate from emp where ename = 'ALLEN') -- 1981/02/20 00:00:00
        order by hiredate desc;

-- Ex. 매니저가 KING인 직원들의 사번, 이름, 매니저 사번을 검색.
select empno, ename, mgr from emp where mgr =
    (select empno from emp where ename = 'KING'); -- KING empno 7839.

-- Ex. ACCOUNTING 부서에 일하는 직원들의 이름, 급여, 부서번호를 검색.
select ename, sal, deptno from emp where deptno =
    (select deptno from dept where dname = 'ACCOUNTING'); -- ACCOUNTING deptno 10
