...
insert into ex_test_byte values ('한글'); --> 실패 (utf-8에서 한글은 한 글자에 3byte 사용)
...
--> tno에는 null이 삽입됨.
insert into ex_test_default (tno) values (1234); -- date를 insert하지 않아도 default 값의 결과가 나옴.
...
/*
 * 테이블을 생성할 때 제약조건(constratint) 만들기:
...
create table ex_emp1 (
    ...
    ename   varchar2(10) not null, -- 이름. -- 단위 안 쓰면 byte
...
insert into ex_emp1 (eno, ename, email) values (1004, 'Johne Doe', 'hgd@itwill.com');
--> unique 제약조건 위배
...
create table ex_emp3 (
    ...
    constraint ex_emp3_nn_ename check (ename is not null), -- 주의! not null은 컬럼이랑 제약조건 정의를 같이 할 때만 가능!
