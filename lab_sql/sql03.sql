 ...
 */
 ...
 -- 직원 테이블에서 수당(comm) null이 아닌 직원들의 사번, 부서번호, 이름, 급여, 수당을 출력. 사번 오름차순 정렬.
 ...
 -- 직원 테이블에서 급여가 2000 이상인 직원들의 이름, 업무, 급여를 출력 -- 급여 내림차순 정렬.
 select ename, job, sal from emp where sal >= 2000 order by sal desc;
 
 -- 직원 테이블에서 급여가 2000 이상 3000 이하인 직원들의 이름, 업무, 급여를 검색. 급여 내림차순 정렬.
 ...
 -- 직원 테이블에서 업무가 'CLERK'인 직원들의 업무, 이름, 급여를 출력. 정렬 순서: 이름.
 select job, ename, sal from emp where job = 'CLERK' order by ename; -- 문자열(' ')은 대/소문자 구분함.
 --> SQL에서는 문자열을 비교할 때 =, != 연산자를 사용.
 --> SQL에서 문자열은 작은따옴표('')를 사용. 대소문자를 구분.
 
 -- 직원 테이블에서 업무가 'CLERK' 또는 'MANAGER'인 직원들의 업무, 이름, 급여를 검색. 정렬 순서: (1) 업무, (2) 급여.
 ...
 -- 직원 테이블에서 20번 부서에서 근무하는 CLERK의 모든 정보(컬럼)을 검색.
 select * from emp  where deptno = 20 and job = 'CLERK';

 
 -- 직원 테이블에서 CLERK, ANALYST, MANAGER가 아닌 직원들의 사번, 이름, 업무, 급여를 검색. 정렬 순서: 사번.
 ...
 -- '1987/01/01' 이후에 입사한 직원들의 모든 정보(컬럼)을 출력. 정렬 순서: 입사일 오름 차순.
