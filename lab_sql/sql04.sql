 ...
 * 오라클 함수(function)
 * 1. 단일 행 함수:
 *      행(row)이 하나씩 함수의 아규먼트로 전달되고, 행마다 하나씩 결과가 리턴되는 함수.
 ...
 */
 ...
 -- nvl(변수, 값): 변수가 null이면 값을 리턴하고, null이 아니면 원래 값을 리턴. 변수에 컬럼이름을 적음.
 -- nvl2(변수, 값1, 값2): 변수가 null이 아니면 값1을 리턴하고,
 -- 변수가 null이면 값2를 리턴.
 select comm, nvl(comm, 0), nvl2(comm, comm, 0), nvl2(comm, 'T', 'F') from emp;
 ...
 select * from emp where lower(ename) like 'a%';
  ...
  -- 사번, 이름, 급여, 수당, 연봉을 검색.
  -- 연봉 = 급여 * 12 + 수당.
  select empno, ename, sal, comm, sal * 12 + nvl(comm, 0) as "연봉" from emp;
 ...
 -- select sal, sum(sal) from emp; sal은 행 14개를 출력, sum(sal)은 행 1개를 출력. 출력하는 행의 개수가 달라서 오류 뜸.
  ...
  * (예) 부서별 직원수, 부서별 급여 평균, ...
  * (문법)
  * select 컬럼, 함수호출, ... from 테이블 where 조건식(1) group by 컬럼(그룹을 묶을 수 있는 변수) having 조건식(2) order by 컬럼(정렬 기준), ...;
  *
  ...
  */
  ...
  -- 부서별 급여 평균
  select deptno, round(avg(sal), 2) as AVG_SAL from emp group by deptno order by deptno;
...
-- Ex. 업무별 업무, 직원수, 급여 최댓값, 최솟값, 평균을 업무 오름차순으로 출력.
 select job, count(job) as COUNT, max(sal) as MAX_SAL, min(sal) as MIN_SAL, round(avg(sal), 2) as AVG_SAL from emp group by job order by job;
 
-- Ex. 부서별/업무별로 부서번호, 업무, 직원수, 급여 평균을 검색.
--     정렬 순서: (1) 부서번호 (2) 업무
 select deptno, job, count(job) as COUNT, round(avg(sal), 2) as AVG_SAL from emp group by deptno, job order by deptno, job;
select deptno, job, count(*) as COUNT, round(avg(sal), 2) as AVG_SAL from emp group by deptno, job order by deptno, job;
 
-- Ex. 입사연도별 사원수를 검색. (힌트) to_char(날짜, 포맷) 이용.
 select to_char(hiredate, 'YYYY') as YEAR, count(hiredate) as COUNT from emp group by to_char(hiredate, 'YYYY') order by to_char(hiredate, 'YYYY');
 select to_char(hiredate, 'YYYY') as YEAR, count(hiredate) as COUNT from emp group by to_char(hiredate, 'YYYY') order by YEAR;
 -- select에서 만들어진 별명은 order by에서 사용 가능.
 
-- Ex. 부서별 급여 평균 검색. 급여 평균이 2000 이상인 부서만 검색.
 select deptno, round(avg(sal), 2) as AVG_SAL from emp group by deptno having round(avg(sal), 2) >= 2000;
...
--     정렬순서: 부서번호 오름차순.
 ...
 select job, count(job) as COUNT from emp where job != 'PRESIDENT' group by job having count(job) >= 3 order by job;
 select job, count(job) as COUNT from emp group by job having job != 'PRESIDENT' and count(job) >= 3 order by job;
